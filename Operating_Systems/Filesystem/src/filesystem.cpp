    ...
    if (file_name.length() <= 3) {
        if (used_name == -1) {
            ...
            if (new_desc_index == -1) {
                ...
                if (curr_pos / BLOCK_LENGTH != 0) {
                    oft[OFT_DIR_BLK].writeDirToBuffer();
                    oft[OFT_DIR_BLK].seek(0, curr_pos - 1, mem.getDescriptor(DIR_DESC)); 
                ...
                oft[OFT_DIR_BLK].setCurrentPos(new_desc_index * SLOT_SIZE);
            ...
            setResponse("file already exists");
        } 
    } else {
...
void FileSystem::deleteFile(vecstr *in) {
    ...
    if (file_name.length() <= 3) {
        ...
        if (ptr_to_filename != -1) {
            bool exists = false;
            for (int i = 1; i < 4; i++) {
                if (file_name.compare(oft[i].getOpenedName()) == 0)
                    exists = true;
            }
            if (exists) {
                setResponse("file is opened cannot delete");
            } else {
...
void FileSystem::openFile(vecstr *in) {
    ...
    if (file_name.length() <= 3) {
        ...
        if (new_oft_index != -1) {
            if (exists) {
                setResponse(file_name + " already opened");
            } else {
                ...
                if (ptr_to_filename == -1) {
                    ...
                    oft[new_oft_index].addFileName(file_name);
...
void FileSystem::closeFile(vecstr *in) {
    ...
    if (isInteger(oft_i)) {
        ...
        if (oft_index >= 1 && oft_index <= 3) {
            if (oft[oft_index].getIsEmpty()) {
                setResponse("index " + std::to_string(oft_index) + " not curently opened");
            } else {
                ...
                oft[oft_index].makeNull();
                setResponse("closed " + oft_i);
            }
        } else {
            setResponse("not a valid use of cl");
        }
    } else {
        setResponse("need to use an index");
    }
}
...
void FileSystem::read(vecstr *in) {
    ...
    if (isInteger(oft_i) && isInteger(end_p)) {
        ...
        if (oft_index >= 1 && oft_index <= 3) {
            if (oft[oft_index].getIsEmpty()) {
                setResponse("index " + std::to_string(oft_index) + " not currently opened");
            } else {
                std::string str;
    ...
    if (isInteger(oft_i) && isInteger(end_p)) {
        ...
        if (oft_index >= 1 && oft_index <= 3) {
            if (oft[oft_index].getIsEmpty()) {
                ...
                int curr_pos = oft[oft_index].getCurrentPos();
                int length = oft[oft_index].getLength();
                if (end_pos >= FILE_LIMIT || end_pos > length) {
                    setResponse("trying to access past file length or file max size");
                } else {
                    // if the bytes being read goes beyond the block, seek to that new block
                    int desc_index = oft[oft_index].getIndex();   
...
bool FileSystem::doesNameExist(std::string name) {
    int curr_pos = oft[OFT_DIR_BLK].getCurrentPos();
    int desc_index = oft[OFT_DIR_BLK].getDescIndex(DIR_DESC);
    if (curr_pos / BLOCK_LENGTH != 0) {
        oft[OFT_DIR_BLK].seek(0, curr_pos, mem.getDescriptor(0));
        oft[OFT_DIR_BLK].resetFiles();
        oft[OFT_DIR_BLK].readDirFromBuffer();
